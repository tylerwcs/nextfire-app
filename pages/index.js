...
export async function getServerSideProps(context) {
  ...
  //   .limit(LIMIT);
  const ref = collectionGroup(getFirestore(), 'posts'); //grab any documents in any collection that has the name 'posts'
...
export default function Home(props) {
  ...
  const getMorePosts = async () => {
    setLoading(true);
    const last = posts[posts.length - 1]; // last post in array 
    ...
    if (newPosts.length < LIMIT) { // if there are no more posts to load
      setPostsEnd(true);
    }
  };
